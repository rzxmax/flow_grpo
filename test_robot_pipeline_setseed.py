...
def test_sde_vs_ode():
    ...
    for run in range(num_runs):
        set_all_seeds(42)  # 每次都使用相同的种子
        
        # 每次都创建新的管线实例，确保模型参数初始化一致
    ...
    # 相同种子下的确定性和随机模式
    set_all_seeds(42)
    pipeline_det = create_fresh_pipeline()
    det_result = pipeline_det.generate_actions(
        obs=fixed_obs,
        num_inference_steps=num_inference_steps,
        output_type="tensor",
        return_dict=True,
    ...
    set_all_seeds(42)
    pipeline_stoch = create_fresh_pipeline()
    stoch_result = pipeline_stoch.generate_actions(
        obs=fixed_obs,
        num_inference_steps=num_inference_steps,
        output_type="tensor",
        return_dict=True,
